...
RVO는 Callee의 반환 값의 크기만큼, Caller의 스택 중 Callee 반환 값을 받기 위해 할당된 메모리 공간에 함수 반환 값을 효율적으로 전달하는 방법에 관한 것이다.
...
```
위와 같이 x가 어디서 왔는지 알 수 없을 때, Caller의 반환 값 스택에 Callee의 반환 값을 바로 쓸 수 없어 `Copy elision`는 작동하지 않는다. 즉, x와 Caller의 반환 값은 공유할 수 없는 자원이고 서로 다른 life cycle을 가진다.
