...
예를들어 bar의 값이 0일 때 두 개의 쓰레드 A,B가 동시에 1,2,3을 실행한다면 다음과 같고
...
할당량(time slice)을 즉시 포기해야하고 그로인해 비용이 큰 context switching도 
...
(sleep)로 빠져 시간 할당량을 포기하기 보단 활성화 상태를 유지하여 공유 자원을 쓸 수 
