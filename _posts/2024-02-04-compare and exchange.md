...
하지만 다른점은 CompareExchange 함수를 사용한 부분으로 마찬가지로 다른 쓰레드가 공유 자원을 동시에 읽은 뒤 변경할 수 있지만 만약 내가 읽고 변경한 값을 쓰려는데 애초에 읽은 값이 다른 쓰레드에 의해 변경 된 경우라면 이를 감지할 수 있어 처음부터 읽고 쓰기를 다시 시도할 수 있게 된 점이다.
