...
그 행렬을 기준 축으로 로컬 버텍스 좌표를 만들고,

메쉬를 월드에 배치할 때,

기준축으로 사용한 로컬 행렬을 이용해 월드 좌표계로의 변환 행렬을 만든 다음,

각각의 버텍스에 곱하여 월드에 배치할 수 있었다.

하지만 메쉬가 한 개의 행렬이 아닌 여러 개의 행렬에 영향을 받는다면,

메쉬의 버텍스를 월드로 배치하기 위해 다른 방법이 필요해졌다.
