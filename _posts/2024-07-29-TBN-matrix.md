...
픽셀 쉐이더에서 오브젝트 공간상 표현된 법선 벡터를 샘플링하여 

월드 공간으로 변환한 뒤 같은 월드 공간의 빛 방향 벡터와 NDotL을 

구하여 사용하면 안 되는 이유는 무얼까?

만약 오브젝트가 본에 스킨닝 되어 애니메이팅 되는 상황을 가정해 보자.
...
모델링 시점의 법선 벡터이고 잘못된 빛 계산을 초래하게 된다.

텍스쳐 공간은 각 텍셀마다 고유의 법선 벡터를 가지며,

이 법선 벡터는 버텍스가 가리키는 텍스쳐 공간의 기준축으로 부터 
