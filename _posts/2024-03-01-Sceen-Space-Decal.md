...
화면상 박스의 표면과 그 박스안 물체의 표면 2차원 좌표를 구할 수 있다.

하지만, 2차원 좌표계에선 박스와 물체의 표면이 겹치는 좌표만 따로 

분리하여 구할 수 없는데,

이는 표면 좌표를 3차원 공간으로 변환하여 구분 지을 수 있다.

물체가 포함 된 박스의 표면 좌표를 3차원 공간으로 변환 시 

표현 좌표는 그대로 x, y로 맵핑하고 

깊이 값만 화면 좌표계상 표면이 가리치는 씬의 깊이로 변환해 보자.

이는 화면 좌표계상 박스와 물체의 표면이 겹치는 씬의 깊이 값은 

3차원상 박스안이고 겹치지 않는 표면은 3차원상 박스 밖으로 변환 되어

2차원에서 구분 지을 없던 물체와 겹치치 않는 박스의 표면을

3차원 변환을 통해 구분 지을 수 있고

이를 통해 데칼을 물체의 표면에만 입힐 수 있다.
