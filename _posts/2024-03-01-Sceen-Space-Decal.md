...
문양을 그려 넣을 Mesh가 2차원 화면에 그려진 상태에서 화면상 그려 넣을 2차원 표면을 3차원 좌표계로 변환 할 수 있을까?
...
또한 화면상의 점이 박스안의 pixel이라면 그대로 Decal 박스의 로컬 좌표계에 맞춰 uv 좌표로 지정하여 texture의 색상을 읽으면 된다.
