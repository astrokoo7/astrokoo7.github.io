...
컴파일러는 함수 호출 전 함수 인자를 stack frame에 저장하고 함수를 호출하는데 이때 인자 타입이 lvalue reference이면 stack frame에 값을 가리키는 메모리 주소만 할당한다. 반면 인자 타입이 lvalue reference가 아니면 stack frame에 값을 복사한다.
