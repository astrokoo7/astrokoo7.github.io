...
brutal force 방식으로 코드를 짜면 문자열의 문자 하나씩 검색하려는 패턴 문자열의 문자와 일일이 비교 후 다르면 문자열의 다음 문자를 패턴 문자열과 처음부터 다시 검색하면서 진행한다.
...
문자열의 인덱스를 하나씩 밀면서 hash 값을 갱신하여 패턴 문자의 hash 값과 비교 후 같다면 찾는 문자열이 맞는지 확인하여 문자열의 패턴 위치를 돌려주면 된다.
