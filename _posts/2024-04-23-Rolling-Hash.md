...
brutal force 방식으로 코드를 짜면 문자열의 문자 하나씩 검색하려는 패턴 문자열의 문자와 일일이 비교 후 다르면 문자열의 다음 문자를 패턴 문자열과 처음부터 다시 검색하면서 진행하여 입력 문자열과 패턴 문자열이 길어질수록 지수적 시간 복잡도 O(n^2)를 가지게 된다.

보통 입력 문자열과 찾으려는 패턴이 짧은 경우 brutal force 방식을 사용하지만, 리눅스 grep 명령어처럼 아주 큰 입력 문자열에서 긴 문자열 패턴을 찾으려 할 땐 적합하지 않다.

이러한 상황에 적합한 알고리즘으론 KMP, Boyer-Moore 등이 있는데 여기선 Rabin-Karp 으로 알려진 간단한 Rolling Hash에 대해 살펴보려고 한다.
...
문자열의 인덱스를 하나씩 밀면서 hash 값을 갱신하고 패턴 문자의 hash 값과 비교하여 찾는 문자열이 맞는지 확인하여 문자열의 패턴 위치를 돌려주면 된다.
