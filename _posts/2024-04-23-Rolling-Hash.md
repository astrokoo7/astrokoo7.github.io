...
롤링 해쉬 알고리즘은 brutal force 방식의 시간 복잡도 O(n*m)를 입력 문자열과 패턴 문자열에 대한 해쉬 값 비교(for if compare hash)를 통해 최소 O(n+m) 시간 복잡도를 가지도록 개선한다.

입력 문자열과 패턴 문자열의 해쉬 값 생성은 문자열의 각 문자당 ascii code 값으로 할당(encoding)하고 자릿수에 따른 같은 문자를 구분하기 위해 각 문자열 자릿수를 지수값으로 hash 충돌이 덜 발생할 base (밑)을 정해 곱하여 만든다.
