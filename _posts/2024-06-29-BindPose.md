...
> <font size="2"> 
> T_Pose_Bone_Inverse는 T Pose 본의 역행렬, 즉 역수로
> 이해하기 쉽게 -T_Pose_Bone으로 바꿔쓰자
> </font>
...
시작 시점의 T Pose의 역수를 구한 뒤

런타임에 애니메이션에 의해 변경 된 본의 현재 월드 변환 행렬에

역수를 곱하여 Diff를 구한 뒤 T Pose상 버텍스에 적용하여 

오브젝트를 변형하는 부분이다. 

추가로, 4개의 본에 weight 값을 주어 비중을 나눠주는 부분은 큰 틀을 

이해하는데 참고 사항이다.

> <font size="2"> 
> Update 함수의 버텍스 변환은 실제론 유니티 엔진 내부에서 자동으로 실행된다.
> </font>
