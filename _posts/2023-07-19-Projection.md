...
이때 \\(X_{proj}\\)는 깊이(\\({Z_{eye}}\\))가 차수(\\({W_{clip}}\\))인 clip space를 perpective divide된 NDC로 볼 수 있고 또한 직각 삼각형 성질에 따라 정리함으로서 각도에 따른 원근감도 적용 되었음을 알 수 있다.

또한 \\(Z_{proj}\\)은 \\(Z_{near}\\)로 알려진 상수이고 (\\({Z_{eye}}\\))는 (\\({W_{clip}}\\))이니 위 식을 아래와 같이 정리 된다.
