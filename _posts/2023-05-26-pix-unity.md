...
시스템 프로그램을 하다보면 os에 요청하는 시스템 콜을 캡처할 수 있음을 알게 된다.

그래픽 시스템 콜도 마찬가지인데 상태 머신 특성상 콜 하나하나로는 전체적인 렌더링 과정을 파악하긴 어렵다.
...
가령 캡처 시 사용중인 리소스가 몇 개이며 개별 리소스의 속성이 어떻고 어디어디서 사용됐는지 또는 어떤 드로우 콜이 무엇을 어떻게 어디에 그리는지 등 짤막한 글로 쓰기엔 부족하다.
