...
이 데이터를 이용해 월드에 씬 오브젝트를 사용해 배치 할 수 있다.

모델링 시점에 정한 모양이 변하지 않는 

일반 오브젝트의 경우,

모델링 시점에 정한 버텍스에 씬 오브젝트의 월드 변환 행렬을 곱하여
...
모델링 시점에 정한 버텍스에 본의 월드 변환 행렬을 곱하여
...
이는 버텍스에 영향을 미치는 변환 행렬이 하나로

변환 행렬 값의 변경은 모든 버텍스에 동일하게 적용되어 메쉬 자체의 모양은 변하지 않는다.
...
bind pose 는 T pose 와의 차이를 구하기 위한 것이고 

그 구한 차이만큼만 오브젝트 space의 버텍스에 적용한다.

T pose에서 bind pose를 빼고 남은 차이점만 오브젝트 스페이시의 버텍스에 곱해준다.
